...
// 4_Creamos un protocolo para aislar a un variable la URL, para que se pueda inyectar un valor distinto de URL y poder itercambiar de forma sencilla entre el json de producción y de testing
...
extension DataRepository {
    ...
    func getData() throws -> [StarCard] {
        let data = try Data(contentsOf: url) // Data e sun buffer de bits en memoria que permite pasarle difernetes contructores como 'contentsOf' en caso de la url. Es decir lo que hace es cargar los datos en bruto de la URL en memoria. Hay que añadir el 'try', hay que gestionar los errores, ya que s epuede producir un error grave que haga inestable la aplicación.
        // Se puede gestionar de dos formas:
        
        // 1. A nivel de lanzamiento que seria poner:
        /**
            do  {
        ...
        // 2. Pero si quiero que la llamada no sea gestionada aqui, sino mas a delante en otro momento de la aplicación donde yo ya me encargare de ver ese posible error, por ejemplo para comunicarselo a los usuarios a través de una alerta, lo que hago es poner solo el try. De manera que todos los try si dan error va hacer que se propage solo el error y el que llame a 'getData' va a tener que llamarlo con 'try getData', es decir se pasa el error hacia arriba. Es la manera que lo tengo echo en este caso.
...
// 6_El struct tiene el protocolo 'DataRepository' el cual tiene la variable 'url', lo que hace que sea obligatoria implementarla dicha variable en el struct
